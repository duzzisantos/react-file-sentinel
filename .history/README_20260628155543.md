...
Run the following to install package into your project

npm i react-file-sentinel
...
function UploadForm() {
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    results.forEach((r) => {
      if (r.verdict === "clean") {
        ...
        console.log(
          `${r.fileName} quarantined: ${r.verdict} (risk: ${r.riskScore}/100)`,
        );
      }
    });
  };
  ...
  return (
    <div>
      ...
      {reports.map((r) => (
        <div key={r.id}>
          {r.fileName}: {r.verdict} ({r.threats.length} threats, risk{" "}
          {r.riskScore}/100)
        </div>
      ))}
...
function CustomUpload() {
  return (
    <FileScanner apiEndpoint="https://api.example.com/scan" clientId="my-app">
  ...
  return (
    <div className="card p-4">
      ...
      {reports.map((r) => (
        <div
          key={r.id}
          className={`alert alert-${r.verdict === "clean" ? "success" : "danger"} py-2`}
        >
